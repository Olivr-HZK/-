...
DEFAULT_INPUT_PATH = "/app/output/facebook_ai_result.json"
...
def build_facebook_markdown(ai_data: Dict[str, Any]) -> str:
    if not isinstance(ai_data, dict):
        return ""
    lines: List[str] = []
    lines.append("🏁【竞品 Facebook 监控 · 自动播报】")
    report_date = _format_report_date(ai_data)
    lines.append(f"📅 日期: {report_date}")
    lines.append("📎 来源: Facebook 页面")
    lines.append("")
    ...
    for idx, company in enumerate(sorted(companies.keys()), 1):
        ...
        for sub_idx, (title, payload) in enumerate(items, 1):
            ...
            subtitle = "   {idx}) 📘 ".format(idx=sub_idx)
            if game:
                subtitle += f"{game} Facebook 页面"
            else:
                subtitle += "官方 Facebook 页面"
            ...
            community = analysis.get("community_and_engagement") or ""
            actions = analysis.get("direct_action_suggestions") or ""

            if summary:
                lines.append(f"      📝 摘要: {summary}")
            if content_strategy:
                lines.append(f"      📚 内容策略: {content_strategy}")
            if ad_insight:
                lines.append(f"      🎯 广告创意观察: {ad_insight}")
            if community:
                lines.append(f"      🤝 社区与互动: {community}")
            if actions:
...
def main() -> int:
    input_path = os.environ.get("FACEBOOK_AI_INPUT_PATH", DEFAULT_INPUT_PATH)
    webhook = get_feishu_webhook()
    ai_data = load_ai_results(input_path)
    if not ai_data:
        print("⚠️ AI 结果为空或格式不符，未发送推送")
        return 1
    text = build_facebook_markdown(ai_data)
